...
_RELEASE_CHECK_SPEC = importlib.util.spec_from_file_location(
    "opendream_release_check_script",
    REPO_ROOT / "scripts" / "release_check.py",
)
assert _RELEASE_CHECK_SPEC is not None and _RELEASE_CHECK_SPEC.loader is not None
...
class ReleaseCheckLockTests(unittest.TestCase):
    def test_remove_dead_release_lock_unlinks_dead_pid(self) -> None:
        ...
        lock_path = Path(temp_dir.name) / "release-check.lock"
        lock_path.write_text(json.dumps({"pid": 999999, "acquired_at": FIXED_NOW}), encoding="utf-8")

        with patch.object(release_check.os, "kill", side_effect=ProcessLookupError):
            self.assertTrue(release_check.remove_dead_release_lock(lock_path))

        self.assertFalse(lock_path.exists())

    def test_remove_dead_release_lock_keeps_live_pid(self) -> None:
        temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(temp_dir.cleanup)
        lock_path = Path(temp_dir.name) / "release-check.lock"
        lock_path.write_text(json.dumps({"pid": 999999, "acquired_at": FIXED_NOW}), encoding="utf-8")

        with patch.object(release_check.os, "kill", return_value=None):
