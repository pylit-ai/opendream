...
class MemoryCliIntegrationTests(unittest.TestCase):
    ...
    def test_showcase_fixture_validates_coding_agent_story(self) -> None:
        fixture = REPO_ROOT / "opendream" / "fixtures" / "showcase_agent_workspace_memory.jsonl"
    ...
    def test_showcase_demo_creates_before_after_proof_report(self) -> None:
        ...
        self.assertEqual(result["scenario"], "agent-workspace-showcase")
    ...
    def test_activate_cursor_hermes_and_github_copilot_round_trip(self) -> None:
        run_cli("init", "--workspace", str(self.workspace))
        for target in ("cursor", "hermes", "github-copilot"):
        ...
        self.assertTrue((self.workspace / ".hermes.md").exists())
        self.assertIn(
            "BEGIN OPENDREAM MANAGED BLOCK: hermes",
        ...
        self.assertTrue((self.workspace / ".opendream" / "hooks" / "hermes-post-task.sh").exists())
        self.assertTrue((self.workspace / ".opendream" / "hooks" / "github-copilot-pre-task.sh").exists())
        for hook_name in ("cursor-pre-task.sh", "hermes-pre-task.sh", "github-copilot-pre-task.sh"):
