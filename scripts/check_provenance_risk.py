...
SCAN_ROOTS = ("README.md", "docs", "opendream", "scripts", "tests", "docs/provenance.md", "THIRD_PARTY_NOTICES.md")
...
def allowed(rel: str, line: str, label: str) -> bool:
    if rel in ALLOWLIST and label in {"unsupported launch claim", "unattributed long URL"}:
        return True
    if rel == "docs/provenance.md" and "Claude Code" in line and "unpublished" in line:
        return True
    if rel == "docs/claims.md" and label == "unsupported launch claim":
        return True
    if rel == "docs/known-limitations.md" and label == "unsupported launch claim":
        return True
...
def main() -> int:
    if not (REPO_ROOT / "docs/provenance.md").is_file():
        print("docs/provenance.md missing", file=sys.stderr)
        return 1
    problems: list[str] = []
