...
SHOWCASE_SCENARIO = "agent-workspace-showcase"
DEFAULT_DEMO_SCENARIO = "default"
SHOWCASE_FIXTURE = FIXTURE_ROOT / "showcase_agent_workspace_memory.jsonl"
...
def run_showcase_demo(store: MemoryStore, *, now: str | None = None) -> dict[str, Any]:
    ...
    before = retrieve(
        ...
        caller_detail="agent-workspace-showcase baseline",
    )
    ...
    after = retrieve(
        ...
        caller_detail="agent-workspace-showcase seeded recall",
    )
...
def _abstention_cases(store: MemoryStore, *, timestamp: str) -> list[dict[str, Any]]:
    ...
    for probe in SHOWCASE_ABSTENTION_CASES:
        result = retrieve(
            ...
            caller_detail="agent-workspace-showcase abstention control",
        )
...
def _memory_hurt_cases(store: MemoryStore, *, timestamp: str) -> list[dict[str, Any]]:
    ...
    for probe in SHOWCASE_MEMORY_HURT_CASES:
        safe = retrieve(
            ...
            caller_detail="agent-workspace-showcase memory-hurt safe path",
        )
        forced = retrieve(
            store,
            query=probe["query"],
            ...
            caller_detail="agent-workspace-showcase memory-hurt forced contested recall",
        )
...
def _reproducibility_metadata(
    *,
    ...
    return {
        "command": (
            "opendream demo --scenario agent-workspace-showcase "
...
def _showcase_objective() -> dict[str, Any]:
    return {
        "title": "Show that OpenDream turns prior agent workspace history into useful prompt context.",
        "description": (
        ...
        "pipeline": [
            {
                ...
                "output": len(events),
                "evidence": (
                    "Synthetic agent workspace events with preference, environment, failure, fix, "
            ...
            },
            {
                ...
                "evidence": "Selected the memories that should guide the next agent prompt.",
            },
            {
