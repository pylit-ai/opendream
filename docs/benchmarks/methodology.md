...
OpenDream's performance benchmark is a fixture-driven composite evaluation that scores the memory subsystem across seven dimensions. It runs as part of the standard verification pipeline (`make verify`) and produces a machine-readable scorecard.

## Evaluation Dimensions

| Dimension | Weight | What it measures |
|-----------|--------|------------------|
| Write precision | 20% | Ratio of high-signal records to total active records. Noise events should be filtered. |
| Retrieval precision | 20% | Whether expected records appear in top-5 retrieval results for labeled queries. |
| Latency | 15% | Consolidation time. Penalizes runs exceeding 1 second. |
...
| Procedural reuse | 10% | Whether procedural workflow events produce records with extracted workflow steps. |
| Gating accuracy | 10% | Whether short/noise queries are correctly gated (skipped) rather than retrieving irrelevant memories. |

## Scoring Formula

```
weighted_total = write_precision * 0.20
...
All three conditions must be met for a passing scorecard.
...
- **Should-match queries** (4): queries with expected title matches, testing retrieval precision
...
6. Run retrieval queries and score precision
7. Run gating queries and score accuracy
8. Compute latency metrics
9. Produce composite scorecard
