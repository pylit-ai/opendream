...
|-------|------|----------------|
| **A — Capture** | Turn conversation or skill output into **events** and, after `maintain`, **durable** memories. | Yes (the agent or skill that calls `emit-event`). |
...
1. **Workspace:** repository root; pass `--workspace` consistently (see [`docs/agent-integrations.md`](../agent-integrations.md)). For a **single ordered list of commands** from `init` through `tick` (and optional Layer C), use [complete-operator-workflow.md](complete-operator-workflow.md).
...
**Do not** treat automation records in `prepare-context` as canonical product backlog unless you **promote** them via explicit events or accepted specs (see [`docs/agent-integrations.md`](../agent-integrations.md)).
