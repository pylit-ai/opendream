...
|--------------------|------------------|-------------------------|
| **Layer A — Capture** | `emit-event`, then `maintain` | **Your agent** (or you) when producing `emit-event` content. OpenDream only stores and consolidates. |
...
Omit **`--now`** in production unless you are testing (see [agent-integrations.md](../agent-integrations.md)).
...
Scaffold `codex-account`, then follow the README in the scaffolded adapter directory for how the CLI is invoked on trusted hosts. Ingest is still via the direct-report path for that strategy (see [agent-integrations.md](../agent-integrations.md) matrix).
...
| [agent-integrations.md](../agent-integrations.md) | `--workspace`, hooks, execution strategy matrix |
