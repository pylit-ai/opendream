...
OpenDream is maintained as a clean-room implementation. This file records
research inspiration, benchmark-provenance boundaries, and vendored-asset
handling.
...
| Excluded sources | Workspace notes, generated local runtime state, account-specific docs, unpublished third-party internals, prompts, datasets, and fixtures are not sources for OpenDream code. | Excluded by `scripts/check_release_hygiene.sh` |

## Explicit Non-Use Statement

OpenDream does not use unpublished third-party code, prompts, fixtures,
datasets, docs, or generated local runtime artifacts. Compatibility names
describe documented behavior or legacy CLI flags, not copied internals.
