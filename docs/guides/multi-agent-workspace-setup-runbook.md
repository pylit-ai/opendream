...
For product behavior and alternatives, see the repository [README](../../README.md) and [agent-integrations.md](../agent-integrations.md).
...
opendream activation-plan --workspace "$WS" --targets hermes
```
...
opendream activate --workspace "$WS" --targets hermes
...
| Hermes Agent | `hermes`                 | `.hermes.md` (managed block), `.opendream/hooks/hermes-*.sh` |
| OpenClaw    | `openclaw`                | `.openclaw/config.json`, `.openclaw/opendream-event-map.md`, `.opendream/hooks/openclaw-hooks.sh` |

**Instruction-only surfaces (Cursor rules, Copilot instructions, Hermes context):** the host may not run shell hooks automatically. The managed files tell the agent which `sh .opendream/hooks/...` commands to run; follow those instructions in-session. See [agent-integrations.md](../agent-integrations.md).
...
5. In **Cursor** / **Hermes** / **Copilot**, open the managed rule or instruction file and ensure pre/post hook commands are executed around substantive work.
